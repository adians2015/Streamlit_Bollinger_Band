...
with st.form("add_stock_form", clear_on_submit=True):
    ...
    with col1:
        # Instruction updated: User no longer needs to type .NS
        user_input = st.text_input("Enter Symbol (e.g. SBIN, TCS, RELIANCE)", placeholder="Type symbol here...").upper().strip()
    with col2:
        st.write("##") 
        submit_btn = st.form_submit_button("➕ Add")

if submit_btn and user_input:
    # AUTOMATIC .NS APPENDING
    # We check if it already has it (in case user types it anyway)
    if not user_input.endswith(".NS"):
        clean_ticker = user_input + ".NS"
    else:
    ...
    if clean_ticker not in st.session_state.symbols:
        ...
        st.sidebar.warning(f"{clean_ticker} is already in the list.")
...
def fetch_data():
    ...
    for sym in st.session_state.symbols:
        try:
            ...
            results.append({
                "Symbol": sym.replace(".NS", ""), # Remove .NS for a cleaner table display
                "Signal": signal,
                "Price": price,
                "VWAP": vwap,
                "EMA30": e30,
...
if st.session_state.symbols:
    ...
    if data_list:
        ...
        st.info("Searching for data... Ensure the symbol name is correct.")
else:
    st.info("Your list is empty. Type a Nifty symbol above.")
